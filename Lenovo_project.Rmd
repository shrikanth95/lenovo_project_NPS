...
ownership.durations <- c("Less than 3 months", "Between 3 - 6 months", "Between 7 – 12 months","More than 12 months")
...
for(duration in ownership.durations){
  pnps.working.week <- pnps.working.week %>%
...
unique(pnps.working.week$Ownership.period)
...
unique(pnps.working.month.all$Ownership.period)
...
plt <- ggplot() +
  ...
  geom_line(data = psi.working.month.all, mapping = aes(x=month, y=avg.star*20, color="Star Rating * 20"))+
  facet_grid(.~Ownership.period)+
  labs(title="NPS, PSI and Star Rating for All Commertial and SMB Products", color = "Metrics:")+ylab("")+xlab("Time")+
  theme(legend.position = "bottom")
  ggsave(plot = plt,filename = "Proposal/Figures/all_metric_summary_not_consumer.pdf",width = 12, height = 4,units = "in")
```
...
# format.type = 1 # For psi/NPS scores downsampled for each month and product
# format.type = 2 # For psi/NPS scores downsampled for each product
psi.working <- calculate.PSI(cis.working = cis.working, format.type = 1)
...
```{r}
write.csv(psi.working, "RawData/cached_main_psi.csv")
write.csv(pnps.working, "RawData/cached_main_pnps.csv")
write.csv(psi.working[complete.cases(psi.working),], "RawData/cached_main_psi_clean.csv")
