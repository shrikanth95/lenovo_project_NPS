...
Finally, our team analyzed the customer sentiment and comments to identify factors correlated with the highest and lowest satisfaction products.  These results may help identify areas for Lenovo to focus further investments that can improve long-term customer satisfaction. Subsequent sections of this document provide detailed explanations of each phase of our analysis and associated recommendations.
...
source("../LenovoAnalysis/MDP_lenovo_helper.R")
...
\begin{itemize}
  \item Very Low: $pNPS \le 8$
  \item Moderately $Low: 8 < pNPS \le 35$
  \item Normal: $35 < pNPS \le 45$
  \item Moderately High: $45 < pNPS \le 65$
  \item Very High: $65 < pNPS$
\end{itemize}
...
Using the above NPS states, we then calculated the natural evolution of NPS by month (i.e., same epoch as linear model).  This natural evolution is equivalent to
...
We present a framework to use temeletry and survey to make decisions on the releasing software interventions using Markov Decision Processes.  In what follows, we describe the states, actions and compute transition probabilities with intervention.
...
NPS values are charasterized based on the industry considerations given in Table \ref{tab:NPS_table} <!--TO DO--> five 20\% quantiles denoted as $S_S \in \{++,+, 0, -, - -\}$) and is referred to as $P_{S}$.

<<<<<<< HEAD
# ```{r Preparation of survey matrix, echo=FALSE}
<!-- # ======= -->
  - ++: PSI above 65
...
```{r Preparation of sentiment matrix, echo=FALSE, eval=TRUE}
# >>>>>>> 8f7cca6498834e603c898edafceea569965c9927
...
tmp <- survey.consumer.all %>%
  group_by(time = floor_date(as.Date(Date.Survey, format = "%m/%d/%y"), unit = "1 day")) %>%
  calcNPS()
...
knitr::kable(q.1, caption = "0.25, 0.5 and 0.75 Quantiles of the atikmpag.sys driver are as follows.",col.names = "Crash Rate")
```
...
We then define a threshold 50\% quantile level $\gamma$ above which the epoch is labelled as Abnormal operation.  Transition matrices can be constructed by backlogging telemetry data.  
...
knitr::kable(P1 ,  caption = "A combined Telemetry-Survey State transition matrix.",row.names = TRUE)
...
- A shift towards normal operating mode is expected in the telemetry state transition matrix associated with an intervention.  The transition matrix is denoted as $P_{T,1}$.  As the telemetry is not provided, we assume that the transition probabilities corresponding to positive and negative survey will increase and decrease respectively by a predefined factor $\gamma_T$.  Note that $\gamma_T$ can be estimated by observing the percentage of impacted machines due to an intervention.

- A relatively small shift is expected in the survey transition matrix as the population is relatively late to react to improvements.  This is referred to as $P_{S,1}$.The new survey transition matrix can be estimated similarly by choosing the fraction by which probabilities will change.  This is denoted by $\gamma_S$. Note that $\gamma_S<\gamma_T$.  


The joint transition matrix $P_1$ is associated with the combination of $P_{S,1}$ and $P_{T,1}$.

For the purpose of demonstration, we assume the transition probability of pNPS to change as described  in \ref{as_is_MDP} after a software intervention.  Assuming $p=10\%$ reduction in the transition probability of going from low to low, we have the new matrix as in Table \ref{tab:action1_prob1}.  Assuming driver updates will reduce the quantile of percentage of  of percentage of crash rates above the , we expect to observe a decrease in the crash
...
delta <- p * P1 
P2[,1] <- P1[,1] - delta[,1]
P2[,2:n.sent] =P1[, 2:n.sent] + delta[,2:n.sent]/(n.sent-1)
...
A cost $r_k$ is incurred when an intervention $k\in\{1,2\}$ is performed.  The no-action cost matrix that incentivize transitions towards higher survey scores are denoted by $R_{T,S,1}$.
...
The reward matrix $R_{S,T,2}$ associated with an intervention should be calculated based on $P_{1}$ as 

$$R_{S,T,2} = -r_2+R_{S,T,1}$$ 
