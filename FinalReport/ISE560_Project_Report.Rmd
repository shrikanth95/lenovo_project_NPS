...
\cline{3-7}
\multicolumn{1}{c}{}     &                & \multicolumn{5}{c|}{Applicable NPS States}  \\ \hline
\multicolumn{1}{|c|}{ID} & Action         & Very Low & Moderately Low & Normal & Moderately High & Very High \\ \hline
\multicolumn{1}{|l|}{1}  & Do Nothing                  & Y  & Y & N      & N  & N      \\ \hline
\multicolumn{1}{|l|}{2}  & Conduct root cause analysis & Y  & Y & N      & N  & N       \\ \hline
\end{tabular}
\end{table}
...
knitr::kable(PConsumer_A1, row.names=TRUE, caption = "\\label{tab:consumer_dn} Consumer, Do Nothing")
...
For the assumed $P_S$ and $P_T$, the combined state matrix is given in the Appendix is Table.\ref{tab:combined_transition_matrix}.  We refer to the final state as  $\mathcal I \in \{(++,N),(+,N),(0,N),(-,N),(--,N),(++,AN),(+,AN),\\(0,AN),(-,AN),(--,AN)\}$. 
...
As the reward is defined based on being in the state, the reward matrix $R_{S,T,2}$ associated with an intervention can be calculated as $$R_{S,T,2}(s,t->s_1,t_1) = -r_2+R_{S,T,1}(s,t->s_1,t_1)$$ where $(s,t), (s_1,t_1)\in\mathcal I$.
...
delta <- p * P1 
P2[,1] <- P1[,1] - delta[,1]
P2[,2:(n.sent*n.tel)] =P1[, 2:(n.sent*n.tel)] + t(matrix(rep(delta[,1],each = n.sent*n.tel-1), nrow = n.sent*n.tel-1, ncol = n.sent*n.tel))/((n.sent*n.tel)-1)
  
knitr::kable(P2, row.names=TRUE, caption = "\\label{tab:action1_prob1} Consumer, Software Patch")

R = array(c(R.st1, R.st2), c(n.tel*n.sent, n.tel*n.sent, 2))
P = array(c(P1, P2), c(n.tel*n.sent, n.tel*n.sent, 2))
