...
Third, we analyzed the customer sentiment in order to identify series level categories that would need to be improved in order to maximise the pNPS scores.  This was shortlisted based on the first isolating categories that temporally correlated with pNPS scores.  Finally, the overall PSI was calculated for each Category-Series pair and sorted based on correlation and the PSI score.   We then identify categories that would need improvement under each category.

Based on our analysis, we conclude that there is some predictive value in the customer sentiment (PSI and star ratings) data.  However, there is still significant variation in NPS that is not accounted for in the models we developed.  We, therefore, recommend Lenovo begin archiving the telemetry data, and once Lenovo has 6-12 months of historical telemetry data available then build new models that incorporate the telemetry data as well.  We also present a framework for exploiting the large volume of sentiment data to identify potential improvements on the series level.  In order to identify meaningful correlation in PSI and pNPS for each product, it is advisable to have lower bounds on the sentiment observations at least on the series level.
...
We will assume $r1=10$, $r2=5$ and conducting a root cause analysis has cost $c=7$. Note: The combined reward matrices using these parameters can be found in Appendix \ref{app:Rewards}.
...
overall.study2 <- merge(summary.cat2, psi.study2,by = "Category")
...
Second, we isolate on specific categories and observe the PSI scores in the $very$ $low$ state over the entire dataset in the Appendix in Table.\ref{tab:neg.psi}.  The final list of categories that needs to addressed can be deduced by sorting Table.\ref{tab:neg.psi} based on the correlation. The final list of categories that would need improvements are given in the Appendix in Table.\ref{tab:improvements}.  A similar analysis is done for Commercial series and the suggested improvements are given in the Appendix in Table.\ref{tab:improvements2}.
...
```{r}
# head(nps.study[order(nps.study$nps),])
bottom <- overall.study %>% filter(SeriesName ==  "IDEAPAD 100 SERIES"|SeriesName ==  "IDEACENTRE 300 SERIES",correlation>0,psi.all<8)

# head(nps.study2[order(nps.study2$nps),])
bottom2 <- overall.study2 %>% filter(SeriesName ==  "E SERIES"|SeriesName ==  "THINKPAD P SERIES",correlation>0,psi.all<8)

```

Further, this analysis also allows us to pick categories that lead to low PSI an pNPS scores.  The bottom three consumer products in terms of total pNPS include Ideapad 120s, Ideacentre 310 and Ideapad 110.  Table.\ref{tab:con_lownps} shows that reliability, power supply, network and Ports/Slots require attention for the Ideapad 100 series. For Ideacentre 300 Series, there was not enough data to observe any meaningfull correlation of categories to pNPS. Further, the existing PSI scores were generally obsered to be in the Normal range.

The bottom three commercial products in terms of total pNPS include products from the E Series and Thinkpad P series.  Table.\ref{tab:com_lownps} shows that drivers require attention for P series and fan, network card and general service required attention for E series.  

\section{Conclusion and Recommendations}

This analysis has led to the creation of two seperate models that can each predict an NPS score. These two models have different predictors and should not be mixed up. Lenovo should choose which model to use based on the type of business group they are analyzing (commercial or consumer). 

The Markov Decision Process will be most useful for Lenovo when they want to determine how much money they can save by fixing product features through different options. We were unable to run a proper simulation of the MDP since we weren't given data about the costs of broken machines or how much it may cost to fix them. However, we included a section on how the MDP would work if these numbers were provided. 

Our primary recommendation is that Lenovo starts archiving their telemetry data. One month's worth of data is not even to build any concrete model. The Markov decision process framework we created could be very useful to Lenovo if this archive of data is established.
...
\section{Recommendation Analysis}
...
## Summary of all relevant categories that need to be addressed that positively impact pNPS scores
...
## Categories that need mprovement for bottom Series with low pNPS scores

```{r}
bottom <- overall.study %>% filter(SeriesName ==  "IDEAPAD 100 SERIES"|SeriesName ==  "IDEACENTRE 300 SERIES",correlation>0,psi.all<8)
knitr::kable(bottom[,c(1,4,8)], col.names = c("Category","$\\rho$","PSI"), caption = "\\label{tab:con_lownps} Categories that need to be improved For Ideapad 100 Series",  row.names = FALSE)
bottom2 <- overall.study2 %>% filter(SeriesName ==  "E SERIES"|SeriesName ==  "THINKPAD P SERIES",correlation>0,psi.all<8)
knitr::kable(bottom2[,c(1,2,4,8)], col.names = c("Category","Series","$\\rho$","PSI"), caption = "\\label{tab:com_lownps} Categories that need to be improved For E and P series",  row.names = FALSE)
