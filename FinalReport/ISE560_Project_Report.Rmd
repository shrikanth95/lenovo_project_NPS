...
```{r, eval=FALSE} 
# TO DO: This snippet is preventing the tex file from compiling althgouh the code runs correctly.  
# Define NPS states
NPS_VERY_HIGH <- 60.0
NPS_MOD_HIGH <- 45.0
NPS_NORMAL <- 35.0
...
NPS values are charasterised based on the industry considerations given in Table \ref{tab:NPS_table} five 20\% quantiles denoted as $S_S \in \{++,+, 0, -, - -\}$) and is referred to as $P_{S}$ where:
...
#TO DO:  Something not right with line 252, setting chunk eval=TRUE for now so I can
# knit the rest of the file
n.sent = 5
tmp <- survey.consumer.all %>%
  group_by(#ProductName,
           time = floor_date(as.Date(Date.Survey, format = "%m/%d/%y"), unit = "1 day")) %>%
  calcNPS()
...
# setting eval=TRUE for now until I get csv files so I can knit the rest of the file
...
#TO DO: setting eval=TRUE for now so I can knit the rest of the file.
n.tel = 2
...
```{r, echo= FALSE, eval=TRUE}
# TO DO: setting eval=TRUE so I can knit the rest of the file
