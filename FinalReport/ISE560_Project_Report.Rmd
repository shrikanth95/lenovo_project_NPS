...
knitr::kable(yogac930.summ, caption="Yoga C930 Predictions and 95% Prediction Interval")
```
...
knitr::kable(ideapad330.summ, caption="Ideapad 330 Predictions and 95% Prediction Interval")
```
...
knitr::kable(yoga730.summ, caption="Yoga 730 Predictions and 95% Prediction Interval")
```
...
knitr::kable(legiony730.summ, caption="Legion Y730 Predictions and 95% Prediction Interval")
...
knitr::kable(t490.summ, caption="T490 Predictions and 95% Prediction Interval")
...
```{r eval=TRUE, fig.cap = "\\label{fig:neg-corr2} Correlation between NPS and PSI for Commercial Series", fig.width = 8}
# slice.low %>% ggplot(aes(x = psi, y = nps, group = SeriesName))+geom_point()+geom_smooth(method="lm", se=FALSE, color="darkgrey")+facet_wrap(.~Category)
...
knitr::kable(summary,  caption = "\\label{tab:corr_category} Category-Sentiment Correlation for Consumer Series ($\\rho$)",  row.names = FALSE,col.names = c("Series", "$\\rho\\ge 0.3$", "$\\rho$ < -0.3"))
```
...
knitr::kable(summary,  caption = "\\label{tab:corr_category2} Category-Sentiment Correlation for Commercial Series ($\\rho$)",  row.names = FALSE,col.names = c("Series", "$\\rho\\ge 0.3$", "$\\rho$ < -0.3"))
```

\subsection{Categories with Negative PSI}

```{r fig.pos="H"}
knitr::kable(neg.psi,col.names = c("Category","Series", "$\\rho$","PSI"), caption = "\\label{tab:neg.psi} Categories with strong correlation to NPS scores for Consumer Series ($\\rho$)",  row.names = FALSE)
```
...
\subsection{Relevant Categories that Positively Correlate with NPS Scores}

```{r}
knitr::kable(imp,col.names =  c("Category","Series", "$\\rho$","PSI"), caption = "\\label{tab:improvements} Categories for Potential Improvement, Consumer Series")

knitr::kable(imp2,col.names =  c("Category","Series", "$\\rho$","PSI"), caption = "\\label{tab:improvements2} Categories for Potential Improvement, Commercial Series")

```


\subsection{Categories for Potential Improvement}

```{r}
bottom <- overall.study %>% filter(SeriesName ==  "IDEAPAD 100 SERIES"|SeriesName ==  "IDEACENTRE 300 SERIES",correlation>0,psi.all<8)
knitr::kable(bottom[,c(1,4,8)], col.names = c("Category","$\\rho$","PSI"), caption = "\\label{tab:con_lownps} Categories for Potential Improvement, Ideapad 100 Series",  row.names = FALSE)
bottom2 <- overall.study2 %>% filter(SeriesName ==  "E SERIES"|SeriesName ==  "THINKPAD P SERIES",correlation>0,psi.all<8)
knitr::kable(bottom2[,c(1,2,4,8)], col.names = c("Category","Series","$\\rho$","PSI"), caption = "\\label{tab:com_lownps} Categories for Potential Improvement, E and P series",  row.names = FALSE)
