...
# Key Variables, Parameters and Critical Uncertainties \ref{critical_analysis}
...
Key parameters include the following:
  ...
  - NPS and PSI Transition Probabilities (estimated from the data)

\textbf{Note sure on how to present telemetry---due to time lag}
...
NPS State          | Range          |Star State          | Range 
----------         | -------------- |----------          | ----------
Very Low NPS       | -100-8         | Negative Star      |  1,2           |
Moderately Low NPS |  8-35          | Neutral Star       |  3             | 
Normal  NPS        |  35-45         | Positive Star      |  4,5           |
Moderately High NPS|  45-60         |
Very High NPS      |  60-100        |

In order to use NPS scores to make decisions on actions--with associated costs--like performing call backs, rolling out software patches and doing nothing, we use Markov Decision Processes to develop an optimial policy for each state of pNPS. Existing pNPS data can be used to calculate state transitions probabilities.  The validity of the regression model described in Section \ref{critical_analysis} can be established by running an MDP using the transition probabilities calculated from the predicted NPS.  

# Preliminary Data Analysis \label{data_analysis}

Series    | Number of Epochs | p-value | $R^2$
---------- | -------------- | ------------ | ------- 
Lenovo AR   | 4 | 0.17 | 0.67     |
Accessories | 5 | 0.42 | 0.49     |
IDEACENTER 300 | 9 | 0.0047 | 0.39 |
IDEAPAD 100 | 13 | 0.36 | 0.33 |
...
p1 <- ggplot() +
  ...
  #labs(title="By Month") +
  theme(legend.position = "none")+ylab("")+xlab("Month")
