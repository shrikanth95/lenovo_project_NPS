...
We elaborate on the key features that we intened to use for the prediction problem.  Based on the preliminary data analysis key variables ($\theta$) include the following:$\theta$ = \{Product Sentiment Index (PSI), Star rating, Business Segment (i.e., Commercial and Consumer), Product Family (Series),Battery Life\}

We observe that the business segment plays a role in the quality and the life cycle of the products as usage patterns for businesses and homes are different.  We also wish to account for the product family/series as different product families can be grouped together.As elaborated in the next section, a key parameter is the transition matrix for star ratings, PSI and pNPS scores.  Probabiities are estimated from the data and specifications are provided in the following section.

Uncertainities feature in the dataset extensively. As NPS scores have a long lead time (3-6 months) before being made available for analysis and as PSI scores are acquired more frequently (weekly), mapping of PSI to NPS should also account for lead time by time shifting data.  Further, there is no concrete evidence of the Sentiment data being geographically close to the NPS survey locations.  Hardware can be subjected to a variety of weather and humidity levels that change the user experience in the long run.  These problems include reduced battery life and touchpads losing sensitivity.  We intend to investigate how we can use telemetry data further.


# Markov Decision Process

The states that can be used to develop a Markov Chain include NPS, star rating and PSI states. We have categorized NPS and PSI into five states and Star rating into three states.\textsuperscript{\ref{refs}}

NPS State          | Range          |Star State          | Range          | PSI State    | Range
----------         | -------------- |----------          | ----------     | ------------ | --------
Very Low NPS       | -100-8         | Negative Star      |  1,2           | Low PSI | -100 to 8 
Moderately Low NPS |  8-35          | Neutral Star       |  3             | Moderately Low NPS |  8-35
Normal  NPS        |  35-45         | Positive Star      |  4,5           | Normal  NPS        |  35-45 
Moderately High NPS|  45-60         |                    |                | Moderately High NPS|  45-60 
Very High NPS      |  60-100        |                    |                | Very High NPS      |  60-100

We consider a two objective problem where we wish to maximise the NPS by taking actions(described shortly) at minimial cost.  We propose to use an MDP where the action space \textit{A} can be defined as \textit{A}=\{Take no action,Roll-out routine software patch,Roll-out Emergency Software Patch,Issue a Product Call-back\}.  Although there is no information on the costs, we can make educated guesses on a relative scale.  For instance, the action with the least cost would be to take no action whereas the most expensive action would be call-back products.  A software patch would fall in the middle with the emergency rollout being more expensive of the two. For the MDP, we propose the two objective problem because minimising only the cost would result in choosing to not act. 

For a pre-defined epoch, transition probabilities of PIS and pNPS can be calculated by measuring the number of times the variables have changed states.  Essentially, for element $p_{i,j}$ of the transition matrix $P_{nps}$, we have
\[p_{i,j} = \frac{\text{Number of transitions from } i \text{ to } j }{\text{Total numer of transitions}}\].  The calculation for PIS and star rating transition probabilities are analogous.  Note that the statisical model predicting pNPS will also have an associated transition matrix $\hat{P}_{nps}$ where individual  probabilities $\hat{p}_{i,j}$are defined as follows:
