...
We consider a two objective problem where we wish to maximize the NPS by taking actions(described shortly) at minimal cost.  We propose to use an MDP where the action space \textit{A} can be defined as \textit{A}=\{Take no action,Roll-out routine software patch,Roll-out Emergency Software Patch,Issue a Product Call-back\}.  Although there is no information on the costs, we can make educated guesses on a relative scale.  For instance, the action with the least cost would be to take no action whereas the most expensive action would be call-back products.  A software patch would fall in the middle with the emergency roll-out being more expensive of the two. For the MDP, we propose the two objective problem because minimizing only the cost would result in choosing to not act. 
...
\[p_{i,j} = \frac{\text{Number of transitions from } i \text{ to } j }{\text{Total numer of transitions}}\].  The calculation for PIS and star rating transition probabilities are analogous.  Note that the statistical model predicting pNPS will also have an associated transition matrix $\hat{P}_{nps}$ where individual  probabilities $\hat{p}_{i,j}$are defined as follows:
