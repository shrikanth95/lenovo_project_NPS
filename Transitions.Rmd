...
comm.stars.week <- sentiment.comm %>%
  ...
  summarise(pos = sum(3>= Stars.Rating),
            neg = sum(Stars.Rating <=2)) %>%
  mutate(Stars.Rating = 100 * (pos - neg)/(pos +  neg))
```


```{r}
workingstars <- comm.stars.week[1:55,]
workingstars <- cbind(workingstars, data.frame(next.Stars.Rating = comm.stars.week$Stars.Rating[2:56]))
workingstars
```
```{r}
sort(workingstars$Stars.Rating)
```

Stars States

Very Neutral: 0-8
Relatively Neutral: 8.01-19.99
Very Positive: 20-100
